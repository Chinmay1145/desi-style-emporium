...
import { allProducts, Product, getProductsByCategory } from "@/data/products";
...
export default function CategoryPage() {
  ...
  useEffect(() => {
    ...
    // Get products based on category or show all if no category
    let filteredProducts;
    if (categoryId) {
      filteredProducts = getProductsByCategory(categoryId);
      // If no categoryId, check search params
    } else {
  ...
  return (
    <div className="container mx-auto px-4 py-8 fade-in">
      {/* Category Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-serif font-bold mb-2">{formatCategoryName(categoryId)}</h1>
        <p className="text-muted-foreground">
          Discover our collection of {formatCategoryName(categoryId).toLowerCase()}.
        </p>
      </div>
